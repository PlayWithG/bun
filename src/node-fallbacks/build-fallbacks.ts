...
for (let fileIndex = 0; fileIndex < allFiles.length; fileIndex++) {
  ...
  const buildCommand =
    Bun.$`${process.execPath} build --define=process.env.NODE_DEBUG:"false" --define=process.env.READABLE_STREAM="'enable'" --define=global:globalThis --outdir=${outdir} ${name} --minify-syntax --minify-whitespace --format=${name.includes("stream") ? "cjs" : "esm"} --target=node ${{ raw: externalModules }}`.text();
