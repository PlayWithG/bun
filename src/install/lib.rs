...
impl RunCommand {
    ...
    /// Returns `<temp>/bun-node-<sha>` (or the debug variant). Android and
    /// Termux honor `BUN_TMPDIR` through the resolver's shared temp-dir logic.
    ///
    ...
    #[cfg(not(windows))]
    pub fn bun_node_dir() -> &'static [u8] {
        static ONCE: std::sync::OnceLock<Box<[u8]>> = std::sync::OnceLock::new();
        ONCE.get_or_init(|| {
            ...
            if bun_core::env::IS_DEBUG {
                path.extend_from_slice(b"bun-node-debug");
            } else {
                path.extend_from_slice(b"bun-node");
                if !bun_core::env::GIT_SHA_SHORT.is_empty() {
                    path.push(b'-');
                    path.extend_from_slice(bun_core::env::GIT_SHA_SHORT.as_bytes());
                }
            }
    ...
    pub(crate) fn bun_node_dir_z() -> &'static ZStr {
        ...
        static ONCE: std::sync::OnceLock<Box<[u8]>> = std::sync::OnceLock::new();
        let terminated = ONCE.get_or_init(|| {
            let mut value = path.to_vec();
            value.push(0);
            value.into_boxed_slice()
        });
        unsafe { ZStr::from_raw(terminated.as_ptr(), terminated.len() - 1) }
    }
    ...
    pub fn create_fake_temporary_node_executable(
        ...
        #[cfg(not(windows))]
        {
            ...
            let argv0_z: &ZStr = if !optional_bun_path.is_empty() {
                ...
                match bun_core::self_exe_path() {
                    ...
                    result => {
                        ...
                        if argv0_bytes.starts_with(Self::bun_node_dir()) {
            ...
            #[cfg(bun_debug)]
            {
                ...
                let _ = bun_sys::delete_tree_absolute(Self::bun_node_dir());
            }

            let dir_z = Self::bun_node_dir_z();
            let mut node_path_buf = bun_paths::PathBuffer::uninit();
            let mut bun_path_buf = bun_paths::PathBuffer::uninit();
            let node_link = bun_paths::resolve_path::join_abs_string_buf_z::<
                bun_paths::platform::Auto,
            >(Self::bun_node_dir(), &mut node_path_buf, &[b"node"]);
            let bun_link = bun_paths::resolve_path::join_abs_string_buf_z::<
                bun_paths::platform::Auto,
            >(Self::bun_node_dir(), &mut bun_path_buf, &[b"bun"]);
            ...
            path.extend_from_slice(Self::bun_node_dir());
            path.push(bun_paths::DELIMITER);
            Ok(())
        }
