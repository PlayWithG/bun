...
impl<'a> Linker<'a> {
    ...
    #[cfg(not(windows))]
    fn rewrite_android_env_shebang(line: &[u8], output: &mut [u8]) -> Option<usize> {
        if !cfg!(target_os = "android") || !line.starts_with(b"#!/usr/bin/env") {
            return None;
        }
        let mut command = &line[b"#!/usr/bin/env".len()..];
        if command
            .first()
        ...
        let split =
            command.starts_with(b"-S") && (command.len() == 2 || command[2].is_ascii_whitespace());
        if split {
            command = &command[2..];
            while command
                .first()
        ...
        let command_len = if command.starts_with(b"bun")
            && (command.len() == 3 || command[3].is_ascii_whitespace())
        {
            3
        } else if command.starts_with(b"node")
            && (command.len() == 4 || command[4].is_ascii_whitespace())
        {
            4
        } else {
            return None;
        ...
        let mut env_path = [0u8; MAX_PATH_BYTES];
        let env_path = if let Ok(prefix) = std::env::var("PREFIX") {
            let candidate = format!("{prefix}/bin/env");
            if candidate.starts_with('/')
                && !candidate
                    .as_bytes()
                    .iter()
                    .any(|byte| byte.is_ascii_whitespace())
                && candidate.len() + 1 < env_path.len()
            {
                env_path[..candidate.len()].copy_from_slice(candidate.as_bytes());
                env_path[candidate.len()] = 0;
                let candidate_z = unsafe { ZStr::from_raw(env_path.as_ptr(), candidate.len()) };
                if sys::is_executable_file_path(candidate_z) {
                    &env_path[..candidate.len()]
                } else if split {
                    return None;
                } else {
                    b"/system/bin/env"
        ...
        let suffix = &line[b"#!/usr/bin/env".len()..];
        let length = 2 + env_path.len() + suffix.len();
        if length > output.len() {
            return None;
        }
        output[..2].copy_from_slice(b"#!");
        output[2..2 + env_path.len()].copy_from_slice(env_path);
        output[2 + env_path.len()..length].copy_from_slice(suffix);
        Some(length)
    }
    ...
    fn try_normalize_shebang(abs_target: &ZStr) {
        ...
        let mut rewritten_shebang_buf = [0u8; MAX_PATH_BYTES];
        let rewritten_shebang_len =
            Self::rewrite_android_env_shebang(chunk_without_newline, &mut rewritten_shebang_buf);
        let has_cr = chunk_without_newline.last() == Some(&b'\r');
        if rewritten_shebang_len.is_none() && !has_cr {
            // Nothing to do!
            return;
        }
        ...
        // Write to temporary file with corrected content
        {
            ...
            let shebang = rewritten_shebang_len
                .map(|length| &rewritten_shebang_buf[..length])
                .unwrap_or(&chunk_without_newline[..chunk_without_newline.len() - 1]);
            if tmpfile.write_all(shebang).is_err() {
                return;
            }
