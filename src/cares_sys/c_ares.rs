...
impl Channel {
    pub fn init<C: ChannelContainer>(this: &C, options: ChannelOptions) -> Option<Error> {
        ...
        let mut opts = Options {
            // Keep the default server enabled because Android may not expose a
            // readable resolver file; `configure_android_resolver_servers` will
            // replace it when Termux or the system configuration is available.
    ...
    #[cfg(target_os = "android")]
    fn configure_android_resolver_servers(channel: *mut Channel) {
        let mut paths = Vec::with_capacity(2);
        if let Some(prefix) = std::env::var_os("PREFIX") {
            paths.push(std::path::PathBuf::from(prefix).join("etc/resolv.conf"));
        }
        paths.push(std::path::PathBuf::from("/etc/resolv.conf"));
        ...
        for path in paths {
            ...
            for line in config.split(|byte| *byte == b'\n') {
                ...
                if fields.next() != Some(b"nameserver") {
                    continue;
                }
                let Some(address) = fields.next() else {
                    continue;
                };
                let mut node: struct_ares_addr_port_node = unsafe { core::mem::zeroed() };
                let mut address_buf = [0u8; 128];
                if address.len() >= address_buf.len() {
                    continue;
                }
                address_buf[..address.len()].copy_from_slice(address);
                address_buf[address.len()] = 0;
                let address_ptr = address_buf.as_ptr().cast::<c_char>();
                if unsafe { ares_inet_pton(AF::INET, address_ptr, node.addr_mut_ptr()) } == 1 {
                    node.family = AF::INET;
                } else if unsafe { ares_inet_pton(AF::INET6, address_ptr, node.addr_mut_ptr()) }
                    == 1
                {
                    node.family = AF::INET6;
                } else {
                    continue;
                }
                node.udp_port = 53;
            ...
            let servers_ptr = servers.as_mut_ptr();
            for index in 0..servers.len().saturating_sub(1) {
                unsafe {
                    (*servers_ptr.add(index)).next = servers_ptr.add(index + 1);
                }
            }
            if unsafe { ares_set_servers_ports(channel, servers.as_mut_ptr()) } == ARES_SUCCESS {
                return;
            }
        }
    }
