...
Bun has no `EventSource` global. To consume an SSE stream from a Bun script, import `EventSource` from `undici`. Bun ships its own `undici` module, so the import works without installing the package.
