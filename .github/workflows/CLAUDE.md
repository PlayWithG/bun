...
- apt.llvm.org stalls now and then. The action's `Setup system deps` step bounds the key download and every apt fetch, and retries the `apt-get update` + `install` round up to three times (`apt-get update` exits 0 when a source fails to download, so the install is what notices). Every job has `timeout-minutes: 30`, about three times the slowest job, so a stall that gets through anyway fails the job instead of holding a runner for hours. cmake and ninja come with the runner image and are not installed.
